...
class FetiSolver():
    ...
    def linear_static(my_system,log = False):
        ...
        command = 'cmd /c mpiexec -l -n ' + str(num_partitions+1) + ' python '+ python_solver_file + ' ' + \
                    filename + ' ' + directory 
...
class FETIsubdomain(Assembly):
    ...
    def solve_local_displacement(self, global_lambda, lambda_id_dict):
    ...
    def calc_null_space(self,solver_opt = 'cholsps'):
        ...
        if solver_opt=='cholsps':
            ...
            num_eigval_to_keep = 3
            
            invval = 1/val[:-num_eigval_to_keep]
            Kinv =  np.matmul( V[:,:-num_eigval_to_keep] ,np.matmul(np.diag(invval),V[:-num_eigval_to_keep,:]))
            
            R = V[:,-num_eigval_to_keep:]
